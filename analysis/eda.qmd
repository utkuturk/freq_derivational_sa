...
# get by item means
avgs <- exp_joined %>%
  group_by(itemNum, suffix, is_sa, logCoord_z) %>%
  dplyr::summarize(
...
ggplot(avgs %>% filter(is_sa == "Suspended"), aes(x = logCoord_z, y = item_mean)) +
  geom_point(aes(color = suffix, shape = is_sa), alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  labs(
    x = "Log Frequency (z-scored)",
    y = "Rating",
    color = "Suffix",
    shape = "Suspension"
  ) +
