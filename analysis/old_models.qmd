...
# Normalize known orthographic variants (expand as needed)
lex <- lex %>%
  mutate(word = dplyr::recode(word,
    "ilim" = "bilim"   # map 'ilim ve teknoloji' to conj1 == 'bilim'
  ))

# Join by the single-noun conj1 to the member word of the pair
exp_joined <- exp %>%
  left_join(lex, by = c("conj1" = "word"))
