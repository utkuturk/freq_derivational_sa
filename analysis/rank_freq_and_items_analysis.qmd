---
title: "Rank Frequency and Item-wise Analysis"
...
- Predictors: full interaction `is_sa * suffix * rank_c`.
- Random effects: `(1 + is_sa * suffix * rank_c | subject) + (1 + is_sa | itemNum)`.
...
# packages
invisible(lapply(c("dplyr", "ggplot2", "tidyr", "readr", "purrr", "stringr", "knitr", "brms", "tidybayes", "loo", "gt"), library, character.only = TRUE))
...
exp_joined$suffix <- factor(exp_joined$suffix, levels = c("-cI", "-lI", "-sIz", "-lIk"))

contrasts(exp_joined$is_sa) <-  contr.sum(2)/2
contrasts(exp_joined$suffix) <- contr.sum(4)/2


colnames(attr(exp_joined$suffix, "contrasts")) <- 
  c("suffixSupportervsHood", "suffixWithvsHood", "suffixWithoutvsHood")
...
run_model_all_suffixes <- function(
  ...
  data$is_sa <- factor(data$is_sa)
  data$suffix <- factor(data$suffix, levels = c("-cI", "-lI", "-sIz", "-lIk"))

  contrasts(data$is_sa) <-  contr.sum(2)/2
  contrasts(data$suffix) <- contr.sum(4)/2
  colnames(attr(data$suffix, "contrasts")) <- 
    c("suffixSupportervsHood", "suffixWithvsHood", "suffixWithoutvsHood")
  ...
  post_fixef <- post_fixef %>%
    ...
    filter(!grepl("^Intercept", term_pretty))
...
We fit a unified models (all suffixes in one model)  with centered rank.

Models use the following prior family (generic `class = "b"` for fixed effects, exponential for sds, LKJ(2) for correlations) and the same random-effects structure. Suffix and `is_sa` are sum-coded (+/-.5), so main effects are grand means and interactions are clean deviations.
...
# Check item-wise differences without grouping suffixes
# check if overall acceptability mattered?
# 
```
